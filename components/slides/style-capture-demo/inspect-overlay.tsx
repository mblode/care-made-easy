...
export const InspectOverlay = ({
  ...
  const updateOverlay = (target: Element, pointerX: number, pointerY: number) => {
    ...
    const labelWidth = panelRect.width || tooltipPanel.scrollWidth || tooltipPanel.offsetWidth;
    const tooltipRect = tooltip.getBoundingClientRect();
    const labelHeight =
      tooltipRect.height || tooltip.scrollHeight || tooltip.offsetHeight || panelRect.height;
    ...
    top = clamp(top, LABEL_VIEWPORT_MARGIN, viewportHeight - labelHeight - LABEL_VIEWPORT_MARGIN);
    ...
    const handleKeyDown = (event: KeyboardEvent): void => {
      ...
      if (event.key === "Shift") {
        ...
        if (parent && parent !== document.documentElement) {
          ...
          updateOverlay(parent, lastPointerRef.current.x, lastPointerRef.current.y);
        }
        return;
      }
      ...
      if (event.key === "Alt") {
        ...
        if (child) {
          ...
          updateOverlay(child, lastPointerRef.current.x, lastPointerRef.current.y);
        }
      }
    };
