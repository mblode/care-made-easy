...
const buildSelector = (element: Element): string => {
  ...
  while (current && current.nodeType === Node.ELEMENT_NODE && segments.length < 4) {
    ...
    const classSelector = classes.map((className) => `.${CSS.escape(className)}`).join("");
...
const sanitizeElement = (element: Element): void => {
  if (shouldOmitElement(element)) {
    element.replaceWith(element.ownerDocument.createComment(element.tagName.toLowerCase()));
    return;
  }
...
const pruneExcludedDescendants = (
  ...
  for (let index = cloneElements.length - 1; index >= 0; index -= 1) {
    ...
    if (shouldOmitElement(sourceEl) || (!includeHidden && isElementHidden(sourceEl))) {
      cloneEl.replaceWith(cloneEl.ownerDocument.createComment(cloneEl.tagName.toLowerCase()));
    }
  }
};
...
const snapshotDefaultStyles = (styles: CSSStyleDeclaration): Record<string, string> => {
...
const createDefaultStyleCache = (): DefaultStyleCache => {
  ...
  const getElementDefaults = (element: Element): Record<string, string> => {
    ...
    const defaults = snapshotDefaultStyles(frameWindow.getComputedStyle(baseline));
    baseline.remove();
...
const snapshotStyles = (
  ...
  const defaultStyles = includeAll ? null : styleCache.getElementDefaults(element);
  ...
  for (const property of properties) {
    ...
    if (parentStyles && INHERITED_PROPERTIES.has(property) && parentStyles[property] === trimmed) {
      continue;
    }
...
export const buildCapture = (root: Element, settings: CaptureSettings): CaptureResult => {
  ...
  try {
    ...
    const captureElement = (element: Element, parentId: string | null): string => {
      ...
      if (settings.includePseudoElements) {
        const before = snapshotPseudoElement(element, "before", includeAll, styleCache);
        const after = snapshotPseudoElement(element, "after", includeAll, styleCache);
        if (before) {
