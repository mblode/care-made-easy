...
export const SyncFlow = ({ animations }: { animations: SyncAnimation[] }) => (
  ...
  >
    <div className="h-full w-0.5 rounded-full bg-[var(--hairline)] md:hidden" />
    <div className="hidden h-0.5 w-full rounded-full bg-[var(--hairline)] md:block" />

    {animations.map((anim) => (
      <div
        key={anim.id}
        className="sync-dot absolute size-2 rounded-full bg-[var(--fg)]"
        style={{
