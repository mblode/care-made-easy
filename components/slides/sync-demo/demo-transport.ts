// oxlint-disable eslint-plugin-promise(avoid-new) eslint-plugin-promise(prefer-await-to-callbacks) eslint(class-methods-use-this) -- TransportAdapter interface requires these patterns
...
export class DemoServer {
  ...
  processMutation(sourceTransportId: string, batch: TransactionBatch): MutateResult {
    const results = batch.transactions.map((tx) => {
      ...
      for (const [id, transport] of this.transports) {
        ...
        if (id !== sourceTransportId) {
          const direction = sourceTransportId === "A" ? "right" : ("left" as const);
...
export class DemoTransport implements TransportAdapter {
  ...
  private flushPendingMutations(): void {
    ...
    for (const { batch, resolve } of queued) {
      this.scheduleResolve(() => resolve(this.server.processMutation(this.transportId, batch)));
    }
  }
  ...
  bootstrap(_options: BootstrapOptions): AsyncGenerator<ModelRow, BootstrapMetadata, unknown> {
  ...
  async *batchLoad(_options: BatchLoadOptions): AsyncGenerator<ModelRow, void, unknown> {
    // No-op for demo
  }
  ...
  fetchDeltas(after: string, _limit?: number, _groups?: string[]): Promise<DeltaPacket> {
